...
- **Observed:** JSON logs with `notification.sent`, `notification.duplicate_ignored`, `notification.retry`, `notification.dead_lettered` lines carrying `eventId`/`messageId` and `correlationId` (the originating `x-request-id`). OpenTelemetry (`src/telemetry.ts`): the consumer span continues the trace of the request that created the order; `notification_messages_total{outcome}` and `notification_processing_duration_seconds`; queue and DLQ depth come from RabbitMQ's own metrics. Dashboards and alerts: `observability/`.
