...
- **Observed:** JSON logs (pino), `req.id` = `x-request-id` (reused or generated, echoed in the response, forwarded to user-service and used as the event `correlationId`); probe requests are not logged; `authorization`/`cookie` redacted. OpenTelemetry (`src/telemetry.ts`): one trace over HTTP, PostgreSQL, Redis, the user lookup and the RabbitMQ publish (the worker continues it); counters for orders, publish failures, user lookups, cache operations, auth refusals and database-unavailable responses. Dashboards and alerts: `observability/`.
