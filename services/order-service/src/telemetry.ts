...
if (!disabled) {
  sdk = new NodeSDK({
    ...
    instrumentations: [
      ...
      // Publish and consume spans; the trace context travels in the message headers, so the worker continues the
      // trace of the request that created the order (OB-2).
      new AmqplibInstrumentation(),
      new IORedisInstrumentation(),
      // fetch() to user-service: injects the trace context, records the client span and duration.
